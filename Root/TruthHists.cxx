...
StatusCode TruthHists::initialize() {
  ...
  unsigned int nBinsN_l = 500; float minBinN_l = -0.5; float maxBinN_l = 699.5;

  m_truth_n             = book(m_name, "truth_n",             "N_{truth}", nBinsN_l, minBinN_l, maxBinN_l );
  m_truth_pt            = book(m_name, "truth_pt",            "truth p_{T} [GeV]", 100, 0, 10 );
  m_truth_pt_m          = book(m_name, "truth_pt_m",          "truth p_{T} [GeV]", 100, 0, 100 );
  m_truth_pt_l          = book(m_name, "truth_pt_l",          "truth p_{T} [GeV]", 100, 0, 500 );
  m_truth_eta           = book(m_name, "truth_eta",           "truth #eta", 80, -4, 4);
  m_truth_phi           = book(m_name, "truth_phi",           "truth #phi",120, -TMath::Pi(), TMath::Pi() );
  ...
  m_truth_pdgId_susy_n  = book(m_name, "truth_pdgId_susy_n",  "truth pdgId_n", 50,-1000000,-1000050 );
  ...
  if( m_detailStr.find("leptons") != std::string::npos ) {
    ...
    m_e_pt             = book(m_name, "truth_e_pt",             "e p_{T} [GeV]", 100, 0, 10 );
    m_e_pt_m           = book(m_name, "truth_e_pt_m",           "e p_{T} [GeV]", 100, 0, 100 );
    m_e_pt_l           = book(m_name, "truth_e_pt_l",           "e p_{T} [GeV]", 100, 0, 500 );
    m_e_eta            = book(m_name, "truth_e_eta",            "e #eta", 80, -4, 4);
    m_e_phi            = book(m_name, "truth_e_phi",            "e #phi",120, -TMath::Pi(), TMath::Pi() );
    m_e_m              = book(m_name, "truth_e_m",              "e m [GeV]", 150, 0, 150 );
    m_mu_n             = book(m_name, "truth_mu_n",             "N_{#mu}", nBinsN, minBinN, maxBinN );
    m_mu_pt            = book(m_name, "truth_mu_pt",            "#mu p_{T} [GeV]", 100, 0, 10 );
    m_mu_pt_m          = book(m_name, "truth_mu_pt_m",          "#mu p_{T} [GeV]", 100, 0, 100 );
    m_mu_pt_l          = book(m_name, "truth_mu_pt_l",          "#mu p_{T} [GeV]", 100, 0, 500 );
    m_mu_eta           = book(m_name, "truth_mu_eta",           "#mu #eta", 80, -4, 4);
    m_mu_phi           = book(m_name, "truth_mu_phi",           "#mu #phi",120, -TMath::Pi(), TMath::Pi() );
    m_mu_m             = book(m_name, "truth_mu_m",             "#mu m [GeV]", 150, 0, 150 );
    m_tau_n            = book(m_name, "truth_tau_n",            "N_{#tau}", nBinsN, minBinN, maxBinN );
    m_tau_pt           = book(m_name, "truth_tau_pt",           "#tau p_{T} [GeV]", 100, 0, 10 );
    m_tau_pt_m         = book(m_name, "truth_tau_pt_m",         "#tau p_{T} [GeV]", 100, 0, 100 );
    m_tau_pt_l         = book(m_name, "truth_tau_pt_l",         "#tau p_{T} [GeV]", 100, 0, 500 );
    m_tau_eta          = book(m_name, "truth_tau_eta",          "#tau #eta", 80, -4, 4);
    m_tau_phi          = book(m_name, "truth_tau_phi",          "#tau #phi",120, -TMath::Pi(), TMath::Pi() );
    ...
    m_lepton_pt        = book(m_name, "truth_lepton_pt",        "lepton p_{T} [GeV]", 100, 0, 10 );
    m_lepton_pt_m      = book(m_name, "truth_lepton_pt_m",      "lepton p_{T} [GeV]", 100, 0, 100 );
    m_lepton_pt_l      = book(m_name, "truth_lepton_pt_l",      "lepton p_{T} [GeV]", 100, 0, 500 );
    m_lepton_eta       = book(m_name, "truth_lepton_eta",       "lepton #eta", 80, -4, 4);
    m_lepton_phi       = book(m_name, "truth_lepton_phi",       "lepton #phi",120, -TMath::Pi(), TMath::Pi() );
  ...
  if( m_detailStr.find("quarks") != std::string::npos ) {
    ...
    m_t_pt            = book(m_name, "truth_t_pt",            "t p_{T} [GeV]", 100, 0, 10 );
    m_t_pt_m          = book(m_name, "truth_t_pt_m",          "t p_{T} [GeV]", 100, 0, 100 );
    m_t_pt_l          = book(m_name, "truth_t_pt_l",          "t p_{T} [GeV]", 100, 0, 500 );
    m_t_eta           = book(m_name, "truth_t_eta",           "t #eta", 80, -4, 4);
    m_t_phi           = book(m_name, "truth_t_phi",           "t #phi",120, -TMath::Pi(), TMath::Pi() );
    m_t_m             = book(m_name, "truth_t_m",             "t m [GeV]", 150, 0, 150 );
    m_b_n             = book(m_name, "truth_b_n",             "N_{b}", nBinsN_m, minBinN_m, maxBinN_m );
    m_b_pt            = book(m_name, "truth_b_pt",            "b p_{T} [GeV]", 100, 0, 10 );
    m_b_pt_m          = book(m_name, "truth_b_pt_m",          "b p_{T} [GeV]", 100, 0, 100 );
    m_b_pt_l          = book(m_name, "truth_b_pt_l",          "b p_{T} [GeV]", 100, 0, 500 );
    m_b_eta           = book(m_name, "truth_b_eta",           "b #eta", 80, -4, 4);
    m_b_phi           = book(m_name, "truth_b_phi",           "b #phi",120, -TMath::Pi(), TMath::Pi() );
    m_b_m             = book(m_name, "truth_b_m",             "b m [GeV]", 150, 0, 150 );
    m_quark_n         = book(m_name, "truth_quark_n",         "N_{quark}", nBinsN_l, minBinN_l, maxBinN_l );
    m_quark_pt        = book(m_name, "truth_quark_pt",        "quark p_{T} [GeV]", 100, 0, 10 );
    m_quark_pt_m      = book(m_name, "truth_quark_pt_m",      "quark p_{T} [GeV]", 100, 0, 100 );
    m_quark_pt_l      = book(m_name, "truth_quark_pt_l",      "quark p_{T} [GeV]", 100, 0, 500 );
    m_quark_eta       = book(m_name, "truth_quark_eta",       "quark #eta", 80, -4, 4);
    m_quark_phi       = book(m_name, "truth_quark_phi",       "quark #phi",120, -TMath::Pi(), TMath::Pi() );
  ...
  if( m_detailStr.find("bosons") != std::string::npos ) {
    ...
    m_Z_pt            = book(m_name, "truth_Z_pt",            "Z p_{T} [GeV]", 100, 0, 10 );
    m_Z_pt_m          = book(m_name, "truth_Z_pt_m",          "Z p_{T} [GeV]", 100, 0, 100 );
    m_Z_pt_l          = book(m_name, "truth_Z_pt_l",          "Z p_{T} [GeV]", 100, 0, 500 );
    m_Z_eta           = book(m_name, "truth_Z_eta",           "Z #eta", 80, -4, 4);
    m_Z_phi           = book(m_name, "truth_Z_phi",           "Z #phi",120, -TMath::Pi(), TMath::Pi() );
    ...
    m_W_pt            = book(m_name, "truth_W_pt",            "W p_{T} [GeV]", 100, 0, 10 );
    m_W_pt_m          = book(m_name, "truth_W_pt_m",          "W p_{T} [GeV]", 100, 0, 100 );
    m_W_pt_l          = book(m_name, "truth_W_pt_l",          "W p_{T} [GeV]", 100, 0, 500 );
    m_W_eta           = book(m_name, "truth_W_eta",           "W #eta", 80, -4, 4);
    m_W_phi           = book(m_name, "truth_W_phi",           "W #phi",120, -TMath::Pi(), TMath::Pi() );
    ...
    m_h_pt            = book(m_name, "truth_h_pt",            "h p_{T} [GeV]", 100, 0, 10 );
    m_h_pt_m          = book(m_name, "truth_h_pt_m",          "h p_{T} [GeV]", 100, 0, 100 );
    m_h_pt_l          = book(m_name, "truth_h_pt_l",          "h p_{T} [GeV]", 100, 0, 500 );
    m_h_eta           = book(m_name, "truth_h_eta",           "h #eta", 80, -4, 4);
    m_h_phi           = book(m_name, "truth_h_phi",           "h #phi",120, -TMath::Pi(), TMath::Pi() );
  ...
  if( m_fillSUSY || (m_detailStr.find("susy") != std::string::npos) ) {
    ...
    m_C1_pt            = book(m_name, "truth_C1_pt",            "C1 p_{T} [GeV]", 100, 0, 10 );
    m_C1_pt_m          = book(m_name, "truth_C1_pt_m",          "C1 p_{T} [GeV]", 100, 0, 100 );
    m_C1_pt_l          = book(m_name, "truth_C1_pt_l",          "C1 p_{T} [GeV]", 100, 0, 1000 );
    m_C1_eta           = book(m_name, "truth_C1_eta",           "C1 #eta", 80, -4, 4);
    m_C1_phi           = book(m_name, "truth_C1_phi",           "C1 #phi",120, -TMath::Pi(), TMath::Pi() );
    ...
    m_N2_pt            = book(m_name, "truth_N2_pt",            "N2 p_{T} [GeV]", 100, 0, 10 );
    m_N2_pt_m          = book(m_name, "truth_N2_pt_m",          "N2 p_{T} [GeV]", 100, 0, 100 );
    m_N2_pt_l          = book(m_name, "truth_N2_pt_l",          "N2 p_{T} [GeV]", 100, 0, 1000 );
    m_N2_eta           = book(m_name, "truth_N2_eta",           "N2 #eta", 80, -4, 4);
    m_N2_phi           = book(m_name, "truth_N2_phi",           "N2 #phi",120, -TMath::Pi(), TMath::Pi() );
    ...
    m_N1_pt            = book(m_name, "truth_N1_pt",            "N1 p_{T} [GeV]", 100, 0, 10 );
    m_N1_pt_m          = book(m_name, "truth_N1_pt_m",          "N1 p_{T} [GeV]", 100, 0, 100 );
    m_N1_pt_l          = book(m_name, "truth_N1_pt_l",          "N1 p_{T} [GeV]", 100, 0, 1000 );
    m_N1_eta           = book(m_name, "truth_N1_eta",           "N1 #eta", 80, -4, 4);
    m_N1_phi           = book(m_name, "truth_N1_phi",           "N1 #phi",120, -TMath::Pi(), TMath::Pi() );
    m_N1_m             = book(m_name, "truth_N1_m",             "N1 m [GeV]", 100, 0, 10 );
  ...
  if( m_detailStr.find("substructure") != std::string::npos ) {
    ...
    m_C1_N2_dPhi       = book(m_name, "truth_C1_N2_dPhi",       "#Delta#phi(C1, N2)", 60, 0, TMath::Pi() );
    m_C1_N2_dEta       = book(m_name, "truth_C1_N2_dEta",       "#Delta#eta(C1, N2)", 80, 0, 4 );
    m_C1_N2_dR         = book(m_name, "truth_C1_N2_dR",         "#DeltaR(C1, N2)", 80, 0, 4 );
    m_N1_W_dR          = book(m_name, "truth_N1_W_dR",          "#DeltaR(N1, W)", 80, 0, 4 );
    m_N1_Zh_dR         = book(m_name, "truth_N1_Zh_dR",         "#DeltaR(N1, Z/h)", 80, 0, 4 );
    m_W_Zh_dR          = book(m_name, "truth_W_Zh_dR",          "#DeltaR(W, Z/h)", 80, 0, 4 );
    m_C1_W_decays_dR   = book(m_name, "truth_C1_W_decays_dR",   "#DeltaR q#bar{q} (or l#nu)", 80, 0, 4 );
    m_N2_Zh_decays_dR  = book(m_name, "truth_N2_Zh_decays_dR",  "#DeltaR b#bar{b}", 80, 0, 4 );

    m_C1_W_decays_dR_vs_W_pt    = book(m_name, "truth_C1_W_decays_dR_vs_W_pt", "W p_{T} [GeV]", 100, 0, 500, "#DeltaR q#bar{q} (or l#nu)", 80, 0, 4 );
    m_N2_Zh_decays_dR_vs_Zh_pt  = book(m_name, "truth_N2_Zh_decays_dR_vs_Zh_pt", "Z/h p_{T} [GeV]", 100, 0, 500, "#DeltaR b#bar{b}", 80, 0, 4 );
  }
...
StatusCode TruthHists::execute( const xAOD::TruthParticleContainer* truths, float eventWeight ) {
  ...
  // number of different particle species
  ...
  if( m_fillSubstructure ) {
    ...
    if( m_C1.size() > 0 && m_N2.size() > 0 ) {
      float C1_N2_dPhi = m_C1[0]->phi() - m_N2[0]->phi();
      if( C1_N2_dPhi < 0 )
      ...
      m_C1_N2_dPhi -> Fill( C1_N2_dPhi, eventWeight );
      m_C1_N2_dEta -> Fill( C1_N2_dEta, eventWeight );
      m_C1_N2_dR   -> Fill( C1_N2_dR, eventWeight );
    }

    if( m_N1_from_C1.size() > 0 && m_W_from_C1.size() > 0 )
      m_N1_W_dR    -> Fill( m_N1_from_C1[0]->p4().DeltaR(m_W_from_C1[0]->p4()), eventWeight );

    if( m_N1_from_N2.size() > 0 && m_Zh_from_N2.size() > 0 )
      m_N1_Zh_dR   -> Fill( m_N1_from_N2[0]->p4().DeltaR(m_Zh_from_N2[0]->p4()), eventWeight );

    if( m_W_from_C1.size() > 0 && m_Zh_from_N2.size() > 0 )
      m_W_Zh_dR    -> Fill( m_W_from_C1[0]->p4().DeltaR(m_Zh_from_N2[0]->p4()), eventWeight );

    if( m_C1_W_decays.size() > 1 ){
      m_C1_W_decays_dR -> Fill( m_C1_W_decays[0]->p4().DeltaR(m_C1_W_decays[1]->p4()), eventWeight );
      if ( m_W_from_C1.size() > 0 ){
        m_C1_W_decays_dR_vs_W_pt -> Fill( m_W_from_C1[0]->pt()/1e3, m_C1_W_decays[0]->p4().DeltaR(m_C1_W_decays[1]->p4()), eventWeight );
      }
    }

    if( m_N2_Zh_decays.size() > 1 ) {
      m_N2_Zh_decays_dR -> Fill( m_N2_Zh_decays[0]->p4().DeltaR(m_N2_Zh_decays[1]->p4()), eventWeight );
      if ( m_Zh_from_N2.size() > 0 ){
        m_N2_Zh_decays_dR_vs_Zh_pt -> Fill( m_Zh_from_N2[0]->pt()/1e3, m_N2_Zh_decays[0]->p4().DeltaR(m_N2_Zh_decays[1]->p4()), eventWeight );
      }
    }
  }
...
StatusCode TruthHists::execute( const xAOD::TruthParticle* truth, float eventWeight ) {
  ...
  m_truth_pt_m          -> Fill( pt,    eventWeight );
  ...
  if( m_fillLeptons ) {
    if( TMath::Abs(pdgId) == 11 ) {
      ...
      m_tau_pt_m        -> Fill( pt,    eventWeight );
      ...
      m_lepton_pt_m     -> Fill( pt,    eventWeight );
  ...
  if( m_fillQuarks ) {
    if( TMath::Abs(pdgId) == 6 ) {
      ...
      m_quark_pt_m      -> Fill( pt,    eventWeight );
